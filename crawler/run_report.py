...
def est_tokens(text: str) -> int:
    """
    글이 토큰(글자 조각) 몇 개쯤 되는지 **어림**합니다.

    한글은 대략 한 글자에 토큰 하나꼴입니다. 실제 값은 AI 를 부른 뒤에야
    정확히 알 수 있는데, 부르기 **전에** "이만큼 더 든다" 를 말씀드려야
    하므로 여기서는 어림값을 씁니다.

    ⚠️ 넉넉하게(많게) 잡습니다. 적게 잡았다가 한도를 넘기는 것보다,
       많게 잡아서 조금 일찍 줄이는 편이 안전합니다.
    """
    return len(text)
...
- 원인을 단정하지 마세요. 자료로 알 수 있는 것은 **무엇이 일어났는가**이지
  **왜 일어났는가**가 아닙니다.
  ⚠️ 그렇다고 "자료만으로는 알 수 없습니다" 같은 변명을 적지도 마세요.
     모르는 것은 **그냥 쓰지 않으면 됩니다.** 아는 것만 적으세요.
...
- **출간 시점과 순위의 관계**: 나온 지 얼마 안 된 책(최근 1~3개월)이 오른
  것과, 나온 지 오래된 책이 다시 오른 것은 성격이 다릅니다. 후자는
  역주행입니다. 오래된 책이 계속 상위에 있으면 스테디셀러로 볼 수 있습니다.
...
【지난 리포트를 겹쳐 읽는 법 — 2026-08-12 추가】
자료 뒤에 **지난 며칠치 리포트**가 함께 붙어 있을 수 있습니다.
그것은 당신이 며칠 전에 쓴 글입니다. 이렇게 쓰세요.

1. **같은 말을 또 하지 마세요.** 어제도 그제도 짚은 책을 오늘 또 같은
   말로 짚지 마세요. 여전히 중요하면 **달라진 점**만 말하세요.
     (나쁨) "『○○』이 상위권을 지키고 있습니다." ← 사흘째 같은 문장
     (좋음) "『○○』은 나흘째 5위 안입니다. 어제까지 알라딘만 강했는데
             오늘은 교보에서도 올랐습니다."
2. **지난 「지켜볼 것」의 답을 하세요.** 며칠 전에 "이걸 보면 판가름
   난다" 고 적어 두었는데 오늘 자료로 답이 나왔다면, 그 답을 적으세요.
   **틀렸으면 틀렸다고 적으세요.** 맞은 것만 골라 적지 마세요.
3. **지난 짐작이 빗나갔으면 그것도 사실입니다.** "며칠 전에 ~일 것으로
   봤는데 그렇지 않았습니다" 는 좋은 정보입니다.

🚨 그러나 **억지로 엮지 마세요.**
   특별한 일이 없는 날에 지난 리포트를 끌어와 분량을 채우지 마세요.
   말할 것이 없으면 「지난 리포트 되짚기」 소제목을 아예 쓰지 마세요.
   지난 글에만 있고 오늘 자료에는 없는 책을 **오늘도 있는 것처럼 쓰면
   안 됩니다.** 오늘 자료가 언제나 사실의 기준입니다.
   지난 리포트에 적힌 숫자를 오늘 숫자인 것처럼 옮겨 적지 마세요.

【도서를 언급할 때】
**제목 (출판사, YYYY-MM)** 형식으로 적으세요. 뒤의 날짜가 출간월입니다.
'배본', '출간월', '배본일' 같은 **말은 붙이지 마세요.** 날짜만 적습니다.
자료에 '출간월 모름' 이라고 되어 있으면 **날짜 없이 (출판사)** 만 적으세요.
'미상' 같은 말도 쓰지 마세요. 지어내지도 마세요.

【말투 — 옆자리 동료에게 친절하게 설명하듯】
어려운 말로 유식해 보이려 하지 마세요. 회의실에서 옆자리 동료에게
말로 설명해 주듯이 쉽게 쓰세요. 읽는 분은 마케팅 담당자이지
데이터 분석가가 아닙니다.
- 업계 용어·분석 용어를 굳이 쓰지 마세요. 꼭 써야 하면 괄호로 한 번
  풀어 주세요. 예: 역주행(나온 지 오래된 책이 다시 오르는 것)
- 한 문장은 짧게. 한 문장에 한 가지만 담으세요.
- '~로 판단된다', '~하는 양상을 보인다', '~에 기인한다' 같은 딱딱한
  보고서 말투 대신 '~입니다', '~로 보입니다' 처럼 편하게 쓰세요.
- 숫자만 나열하지 말고, 그 숫자가 **무슨 뜻인지** 한 마디 붙이세요.
  (나쁨) "교보 12위, 알라딘 3위."
  (좋음) "교보에서는 12위인데 알라딘에서는 3위입니다.
          같은 책인데 서점에 따라 반응이 꽤 다릅니다."
- 쉽게 쓴다고 해서 내용을 무르게 하라는 뜻은 아닙니다.
  **짚을 것은 분명하게 짚되, 말을 쉽게** 하라는 뜻입니다.
...
(서점 간 편차, 출간 시점 대비 지금, 진입 깊이 같은 것을 활용하세요)
...
## 지난 리포트 되짚기
⚠️ **이 소제목은 쓸 말이 진짜 있을 때만 쓰세요.** 없으면 소제목째로
빼세요. 억지로 채우지 마세요. 대부분의 날은 이 항목이 없는 것이 정상입니다.

전체 1,000자 안팎. 인사말과 맺음말은 쓰지 마세요.
확신할 수 없는 것은 쓰지 않는 편이 낫습니다.
변명·단서·면책 문장을 넣지 마세요. 아는 것만 담백하게 적으면 됩니다."""


def build_prompt(digest_text: str, history_text: str = "") -> str:
    """
    AI 에게 보낼 글.

    ⚠️ 순서가 중요합니다. **오늘 자료를 먼저, 지난 리포트를 나중에** 둡니다.
       지난 글을 앞에 두면 그 내용에 끌려가서, 오늘 자료에 없는 책 이야기를
       이어서 쓰는 일이 생깁니다. 오늘 자료가 언제나 기준입니다.
    """
    out = (
    ...
    if history_text.strip():
        out += (
            "\n\n"
            "아래는 **참고용**으로 붙인 지난 며칠치 리포트입니다 "
            "(당신이 며칠 전에 쓴 글입니다).\n"
            "🚨 이건 자료가 아닙니다. **여기 적힌 책·숫자를 오늘 것으로 쓰지 "
            "마세요.** 오늘 자료는 위쪽 하나뿐입니다.\n"
            "같은 말을 반복하지 않기 위해서, 그리고 지난 「지켜볼 것」의 답을 "
            "하기 위해서만 쓰세요. 쓸 말이 없으면 안 써도 됩니다.\n\n"
            "=====\n"
            f"{history_text}\n"
            "=====\n"
        )
...
def main() -> int:
    ...
    # -------------------------------------------------------------------------
    #  지난 리포트 붙이기 (2026-08-12 대표님 요청)
    #
    #  "매번 똑같은 말을 할 수도 있기 때문에 그것을 방지하려는 목적도 있고,
    #   이전에 있었던 리포트의 가설이 맞았는지 확인해볼 수도 있고"
    #
    #  🚨 이건 돈이 더 드는 일입니다. 넣는 글이 길어진 만큼 값이 오릅니다.
    #     그래서 **한도를 넘길 것 같으면 날수를 스스로 줄이고, 줄였다고
    #     화면에 적습니다.** 조용히 줄이면 "7일치를 보고 썼겠지" 라고
    #     믿게 됩니다.
    # -------------------------------------------------------------------------
    want_days = int(conf.get("history_days", 7))
    max_chars = int(conf.get("history_max_chars", 1500))
    history = report_data.recent_reports(client, day, want_days)
    hist_text = report_data.history_text(history, max_chars)

    if want_days > 0:
        if not history:
            print(f"\n지난 리포트: 없습니다 (오늘이 처음이거나 아직 쌓이는 중)")
        else:
            add_in = est_tokens(hist_text)
            add_cost = (add_in / MILLION) * price_of(conf, model)[0]
            days_left_now = calendar.monthrange(int(day[:4]), int(day[5:7]))[1] \
                - int(day[8:10]) + 1
            print(f"\n지난 리포트 {len(history)}편을 같이 봅니다 "
                  f"({history[-1]['report_date']} ~ {history[0]['report_date']})")
            print(f"  · 이것 때문에 늘어나는 값: 하루 약 ${add_cost:.4f} "
                  f"({won(add_cost)}) · 남은 {days_left_now}일이면 "
                  f"약 ${add_cost * days_left_now:.2f}")

            # 한도를 넘길 것 같으면 날수를 줄입니다 (그리고 반드시 알립니다)
            room = cap - spent
            if add_cost * days_left_now > room * 0.5 and len(history) > 2:
                keep = max(2, len(history) // 2)
                dropped = len(history) - keep
                history = history[:keep]        # 최신 것부터 남깁니다
                hist_text = report_data.history_text(history, max_chars)
                print(f"  ⚠️ 이대로면 이번 달 한도({won(cap)})에 부담이 커서 "
                      f"{dropped}편을 뺐습니다 → {keep}편만 봅니다.")
                print(f"     더 보시려면 config/report.yaml 의 "
                      f"monthly_cap_usd 를 올리시거나 model 을 "
                      f"claude-haiku-4-5 로 바꾸세요 (값 1/5).")

    # ---- AI 부르기 ----
    print(f"\n{model} 에게 요약을 부탁합니다...")
    body, tin, tout = ask_claude(conf, build_prompt(text, hist_text))
