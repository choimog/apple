...
# 2026-08-09 요청 — 출간월 표기와 '원인 단정 금지'
check("도서 언급 형식이 (출판사, 연-월)", "**제목 (출판사, YYYY-MM)**" in rr.SYSTEM)
...
# 2026-08-10 요청 —
#   "리포트에서 '배본'이라고 굳이 적을 필요는 없어.
#    원인은 자료만으로는 알 수 없다는 고백도 적을 필요는 없어."
# 이 두 가지는 **말로 부탁한 것**이라 조용히 되돌아갈 수 있습니다. 못박아 둡니다.
print("\n[9-1] 2026-08-10 요청 — '배본' 안 쓰기 · 변명 안 쓰기")
ban_lines = [l for l in rr.SYSTEM.splitlines() if "배본" in l]
check("'배본' 은 '쓰지 말라' 는 문장에만 나옴",
      len(ban_lines) == 1 and "붙이지 마세요" in ban_lines[0], ban_lines)
check("'미상' 도 쓰지 말라고 함", "'미상' 같은 말도 쓰지 마세요" in rr.SYSTEM)
check("'자료만으로는 알 수 없다' 는 변명을 금지함",
      "변명을 적지도 마세요" in rr.SYSTEM)
check("모르면 그냥 안 쓰면 된다고 알려줌",
      "그냥 쓰지 않으면 됩니다" in rr.SYSTEM)
check("면책 문장 금지가 맺음말에 있음",
      "변명·단서·면책 문장을 넣지 마세요" in rr.SYSTEM)
# '모르면 모른다고' 규칙(위 [9])과 부딪히지 않아야 합니다.
# 규칙은 남기되 **변명 문장만** 없애는 것이 요청이었습니다.
check("'모르면 모른다' 규칙 자체는 그대로 남아 있음", "알 수 없습니다" in rr.SYSTEM)

print("\n[9-3] 2026-08-10 요청 — 말투는 '옆자리 동료에게 설명하듯'")
# "전문적인 인사이트인 건 좋은데, 너무 알아듣기 어렵게 말하기보다는
#  직장 동료에서 친절하게 설명해주듯이 하면 좋을 것 같아."
check("쉽게 쓰라는 지시가 있음", "옆자리 동료" in rr.SYSTEM)
check("용어를 괄호로 풀어 주라고 함", "괄호로 한 번" in rr.SYSTEM)
check("딱딱한 보고서 말투를 금지함", "~로 판단된다" in rr.SYSTEM)
# ⚠️ '쉽게' 를 '무르게' 로 알아들으면 리포트가 하나 마나 해집니다.
check("쉽게 쓰되 내용은 무르게 하지 말라고 못박음",
      "짚을 것은 분명하게 짚되" in rr.SYSTEM)

print("\n[9-2] 자료에 출간월이 실제로 들어가는지")
# AI 에게 출간월을 적으라고 시켜 놓고 정작 자료에 안 주면,
# AI 는 지어내거나 전부 '미상' 으로 적습니다. 둘 다 나쁩니다.
d_ym = dict(d2)
for r in d_ym["rows"]:
    r["pub_ym"] = "2024-03"
txt = rd.to_text(d_ym)
check("순위표에 출간월이 들어감", "2024-03" in txt, txt[:200])
check("자료에도 '배본' 이라는 말이 없음", "배본" not in txt, txt[:200])
for r in d_ym["rows"]:
    r["pub_ym"] = ""
txt2 = rd.to_text(d_ym)
check("모르면 '출간월 모름' 으로 넘어감", "출간월 모름" in txt2)
