...
# -----------------------------------------------------------------------------
#  🚨 제목 안의 '숫자 + 원○○' 을 가격으로 읽던 것 (2026-08-18 정가 조사에서 발견)
# -----------------------------------------------------------------------------
#  알라딘에 이렇게 저장돼 있었습니다.
#
...
#  위 _PRICE 는 숫자와 '원' 사이의 공백을 허용하는데(태그가 갈라놓는
#  경우가 있어서 필요합니다), 그 때문에 제목의 '숫자 + 띄어쓰기 + 원○○'
#  이 걸려듭니다.
#
#  같은 모양이면 전부 틀립니다 — '2025 원서 읽기', '1000 원의 행복' …
#
#  【어떻게 거르나요】
#  **띄어쓰기가 있고 '원' 뒤에 한글이 바로 이어지면** 그건 값이 아니라
#  낱말의 일부입니다. 값이라면 '원' 에서 끝나거나 뒤에 조사·공백이 옵니다.
#
#  ⚠️ 일부러 좁게 잡았습니다. 붙여 쓴 '12,500원' 은 건드리지 않습니다.
#     넓게 잡으면 '12,500원부터' 같은 멀쩡한 값까지 버립니다.
#     (그건 위 _COND_AFTER 가 이미 조건으로 처리합니다)
# -----------------------------------------------------------------------------
_WORD_AFTER = re.compile(r"^[가-힣]")
...
def parse_prices(text: str | None, list_first: bool = True) -> tuple[
    ...
    for m in _PRICE.finditer(text):
        ...
        if any(w in before for w in _PROMO_WORDS):
            continue
        # 🚨 '2026 원큐패스' 처럼 띄어 쓴 뒤 '원' 에 한글이 이어지면
        #    그건 값이 아니라 낱말입니다 (제목을 가격으로 읽던 것).
        if " " in m.group(0) and _WORD_AFTER.match(text[m.end():m.end() + 1]):
            continue
