...
print("\n[5-1] 🚨 제목 안의 '숫자 + 원○○' 를 값으로 읽지 않는다 (2026-08-18)")
# 【정가 조사에서 발견】 알라딘에 이렇게 저장돼 있었습니다.
#     2026 원큐패스 간호조무사 모의고사 문제집
#         교보 18,000원 · 예스24 20,000원 · 알라딘 2,026원
# 제목의 '2026 원큐패스' 에서 '2026 원' 을 값으로 읽었습니다.
# 숫자와 '원' 사이 공백을 허용하기 때문인데, 그 공백은 태그가 값을
# 갈라놓는 경우 때문에 필요합니다. 그래서 없앨 수 없고, 대신
# '원 뒤에 한글이 바로 이어지면 낱말' 이라는 조건을 더했습니다.
for label, text, want in [
    ("실제 사례", "2026 원큐패스 간호조무사 모의고사 문제집 18,000원 16,200원", 18000),
    ("제목만 있을 때", "2026 원큐패스", None),
    ("원서", "2025 원서 읽기", None),
    ("원의", "1000 원의 행복", None),
    ("원짜리", "5000 원짜리 우산 12,500원", 12500),
]:
    got, _ = parse_prices(text)
    check(f"{label} → 정가 {want!r}", got == want, got)

print("\n[5-2] 🚨 멀쩡한 값을 버리면 안 됩니다 (좁게 잡았는지 확인)")
# 태그가 '12,500' 과 '원' 을 갈라놓으면 '12,500 원' 이 됩니다.
# 이건 진짜 값이므로 살아 있어야 합니다.
for label, text, want_l, want_s in [
    ("붙여 쓴 값", "12,500원", 12500, None),
    ("태그가 갈라놓은 값", "12,500 원", 12500, None),
    ("둘 다 갈라진 값", "12,500 원 11,250 원", 12500, 11250),
    ("앞말이 붙은 값", "정가 12,500원 판매가 11,250원", 12500, 11250),
    ("비싼 책", "180,000원 162,000원", 180000, 162000),
]:
    got_l, got_s = parse_prices(text)
    check(f"{label} → {want_l!r} · {want_s!r}",
          (got_l, got_s) == (want_l, want_s), (got_l, got_s))
