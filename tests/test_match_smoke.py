...
SERIES = [
    ...
    # 🚨 같은 서점(알라딘) 안의 두 번째 상품 — 정가까지 다릅니다.
    #    이 짝은 맨 첫 줄('같은 서점')에서 갈라지므로 정가·권 번호 규칙에
    #    **닿지도 못합니다.** 그런데 2026-08-18 로그에서는 이런 짝이
    #    분모에 98,466쌍이나 들어가 정가 점검이 62.8% 🚨 로 떴습니다.
    book(10, 3, "빛과 수의 시대 3", "김상욱", "동아시아", price=19800),
]
...
check("건수만 적지 않고 분모(나머지가 다 같은 짝)도 적는다",
      "권 번호 점검" in out7 or "갈라낸 짝 없음" in out7, out7[-400:])

print("\n[9-1] 🚨 닿지도 못하는 짝이 정가 점검 분모에 들어가지 않는가")
# 같은 서점끼리는 정가 규칙에 닿지 못합니다. 그런데도 표에 줄이 생기면
# 분모가 '비교해 본 모든 짝' 이라는 뜻이고, 비율은 아무 뜻이 없어집니다.
check("표에 '알라딘↔알라딘' 같은 줄이 없다",
