...
def rpc_all(client, fn: str, params: dict, *,
            step: int = 1000, max_rows: int = 200_000) -> list[dict]:
    """
    **계산 함수(RPC)** 의 결과를 끝까지 받아옵니다.

    🚨 【2026-09-01 실제로 겪은 문제 — 위 _select_all 과 똑같은 함정입니다】
      [도서 목록 정리] 가 매일 성공하는데도 용량이 계속 늘었습니다.
      로그를 보니 이랬습니다.

          ▶ 잠든 상품을 고릅니다 (기준 14일 · 최대 80,000줄)
            잠든 상품 1,000줄          ← 8,975개가 대상인데 1,000개만 왔습니다
            🗑️ 상품 1,000줄을 지웁니다…

      **8만 줄을 달라고 해도 Supabase 는 1,000줄만 돌려줍니다.**
      표를 읽을 때(_select_all)는 이 처리를 해 뒀는데, 계산 함수를 부를
      때는 빠져 있었습니다.

      하루에 새로 들어오는 상품이 2,500~3,000줄인데 1,000줄만 지우니
      매일 순증했습니다. 정리는 '성공' 으로 끝나서 아무 표시도 안 났습니다.
      **조용히 절반만 하는 고장** 이라 로그를 열어 보기 전엔 몰랐습니다.

    ⚠️ 나눠 받을 때마다 계산이 **처음부터 다시** 돕니다. 그래서 함수가
       늘 같은 차례로 돌려줘야 합니다 (ORDER BY 가 있어야 합니다).
       db/prune-catalog.sql 의 두 함수는 `ORDER BY id` 로 되어 있습니다.

    ⚠️ 함수 안의 p_limit 은 여기서 받으려는 양보다 커야 합니다.
       안 그러면 함수가 먼저 잘라서, 그 뒤로는 빈 결과만 옵니다.
    """
    out: list[dict] = []
    start = 0
