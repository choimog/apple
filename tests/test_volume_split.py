...
# =============================================================================
print("\n[8] 🚨 갈라낸 '이유' 의 순서 — 로그의 분모가 여기에 달려 있습니다")
# =============================================================================
#  【2026-08-18 실제 사고】
#  권 번호와 정가는 원래 **제목을 보기도 전에** 갈라내고 있었습니다.
#  그러면 남남인 두 책(제목 앞 네 글자만 같은 책)도 "정가가 다름" 으로
#  갈라집니다. 그 건수를 세었더니 이런 표가 나왔습니다.
#
#      🚨 교보↔예스24: 89,115 / 141,900쌍 (62.8%)
#      ✅ 교보↔교보:        0 /  98,466쌍 (0.0%)   ← 증거
#
#  같은 서점끼리는 첫 줄에서 갈라져 정가까지 오지도 못하는데 분모에는
#  들어 있었습니다. 분모가 '같은 책 후보' 가 아니라 '비교해 본 모든 짝'
#  이었던 것입니다. 같은 날 실제 자료로 센 값은 0.77% 였습니다.
#
#  그래서 둘 다 **다른 조건이 전부 같은 것을 확인한 뒤**로 옮겼습니다.
#  갈라지는 짝은 그대로이고, 기록되는 이유와 로그의 비율만 정확해집니다.

# ① 제목부터 다른 책은 '제목' 으로 갈라야 합니다 (정가가 아니라)
far = judge(("코스모스", "칼세이건", "사이언스북스", "2026-01", 22000),
            ("코딩의 정석", "김코딩", "사이언스북스", "2026-01", 18000))
check("제목이 딴판이면 '제목이 너무 다름' 으로 적는다",
      far.reasons.get("rejected_by") == "제목이 너무 다름", far.reasons)

# ② 같은 서점 짝은 정가·권 번호 칸에 아예 안 잡혀야 합니다 (분모에서 빠짐)
same_store = compare(
    mk(1, "빛과 수의 시대 1", "김상욱", "동아시아", "2026-06", 16000, sb_id=201),
    mk(1, "빛과 수의 시대 2", "김상욱", "동아시아", "2026-06", 19000, sb_id=202),
    CFG)
check("같은 서점 짝은 '같은 서점' 으로 갈라진다",
      same_store.reasons.get("rejected_by") == "같은 서점 안의 두 상품",
      same_store.reasons)
check("같은 서점 짝은 정가 칸에 안 남는다", "price" not in same_store.reasons)
check("같은 서점 짝은 권 번호 칸에도 안 남는다",
      "volume" not in same_store.reasons)

# ③ 나머지가 다 같고 정가만 다르면 그때는 '정가' 로 갈라야 합니다
only_price = judge(("데미안", "헤르만헤세", "민음사", "2026-01", 12000),
                   ("데미안", "헤르만헤세", "민음사", "2026-01", 15000))
check("나머지가 같고 정가만 다르면 '정가가 다름'",
      only_price.reasons.get("rejected_by") == "정가가 다름", only_price.reasons)

# ④ 묶인 짝에는 '같았다' 는 표시가 남아야 합니다 (이게 분모입니다)
ok_pair = judge(("빛과 수의 시대 2", "김상욱", "동아시아", "2026-06", 16000),
                ("빛과 수의 시대 2", "김상욱", "동아시아", "2026-06", 16000))
check("정가가 같으면 same 으로 남는다",
      str(ok_pair.reasons.get("price", "")).startswith("same"), ok_pair.reasons)
check("권 번호가 같아도 same 으로 남는다",
      str(ok_pair.reasons.get("volume", "")).startswith("same"), ok_pair.reasons)
