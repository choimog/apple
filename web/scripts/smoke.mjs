...
const PAGES = [
  ...
  // 2026-08-19 부터 관리자 전용입니다. 보기 전용 계정으로 확인할 때는
  // "관리자만 볼 수 있습니다" 가 뜹니다. 제목은 둘 다 같습니다.
  // (어느 쪽이 떴는지는 아래 [수집 상태] 확인이 따로 봅니다)
  ["/status", ["수집 상태"]],
...
/**
 * 수집 상태가 **둘 중 하나**로 제대로 뜨는지 (2026-08-19 부터 관리자 전용).
 *
 *   관리자      → 수집 기록 표가 보인다
 *   보기 전용   → "관리자만 볼 수 있습니다" 안내가 보인다
 *
 * 🚨 둘 다 아니면 고장입니다. 그냥 제목만 보면 **표도 안내도 없는 빈
 *    화면**을 통과시키게 됩니다. 그게 이 프로젝트에서 제일 위험한
 *    종류입니다 — 화면은 멀쩡한데 아무것도 없는 것.
 */
await (async () => {
  const name = "/status (관리자 전용)";
  try {
    const html = (await (await get("/status")).text()).replaceAll("<!-- -->", "");
    if (html.includes("날짜별 · 서점별 수집 기록")) {
      ok(name, "관리자로 열림 — 수집 기록이 보입니다");
    } else if (html.includes("관리자만 볼 수 있습니다")) {
      ok(name, "보기 전용 계정 — 막혔습니다 (정상)");
    } else {
      bad(name, "수집 기록도 없고 '관리자만' 안내도 없습니다 (빈 화면)");
    }
  } catch (e) {
 ...
 * (보기 전용 계정으로 확인 중이면 링크 자체가 없어서 건너뜁니다)
 */
await (async () => {
