...
def main() -> int:
    ...
    # -------------------------------------------------------------------------
    #  🚨 【2026-08-18 대표님 신고 — 검토 화면이 거짓말을 하고 있었습니다】
    #    "실제로는 안 묶여있는데, 왜 매칭도서 페이지에는 자동으로 묶은
    #     것으로 나오지?"
    #
    #  검토 화면은 book_matches 의 판정('auto_high')을 그대로 읽습니다.
    #  그런데 그 판정은 **무리를 정리하기 전**에 적어 둔 것입니다.
    #  위의 갈라내기(출판사·권 번호·같은 서점·사람 결정)를 거치면서
    #  둘이 다른 무리로 떨어져도, 적어 둔 판정은 그대로 남았습니다.
    #  그래서 화면에는 '자동으로 묶음' 인데 실제로는 안 묶여 있습니다.
    #
    #  여기서 **실제로 한 무리에 남은 짝만** 남깁니다.
    # -------------------------------------------------------------------------
    kept_rows = []
    split_after = 0
    for m in match_rows:
        oa = owner.get(m["store_book_a"])
        ob = owner.get(m["store_book_b"])
        if oa is not None and oa == ob:
            kept_rows.append(m)
        else:
            split_after += 1
    if split_after:
        print(f"  ✂️ 점수는 넘겼지만 무리를 정리하면서 갈라진 짝 "
              f"{split_after:,}쌍 — '자동으로 묶음' 으로 적지 않습니다.")
    match_rows = kept_rows
    ...
    # -------------------------------------------------------------------------
    #  🚨 【2026-08-18 — 더 이상 묶이지 않는 옛 기록을 지웁니다】
    #
    #  저장은 '덮어쓰기(upsert)' 라서 **새 기록만 갱신**합니다. 어제는
    #  묶였는데 오늘은 안 묶이는 짝은 어제 적어 둔 'auto_high' 가 그대로
    #  남습니다. 지우는 코드가 아예 없었습니다.
    #
    #  그래서 검토 화면의 [자동으로 묶은 것] 에는 **이미 갈라진 짝**이
    #  계속 쌓여 있었습니다. 대표님이 보신 것이 이것입니다.
    #  오늘 넣은 권 번호 규칙이 옛 짝을 여럿 갈라내면서 눈에 띈 것입니다.
    #
    #  ⚠️ 사람이 내린 결정(manual_merge / manual_split)은 **읽지도 않습니다.**
    #     "사람이 내린 결정이 최우선" 은 여기서도 지켜야 합니다.
    #  ⚠️ 지워도 잃는 것이 없습니다. 이 표는 매번 처음부터 다시 계산합니다.
    # -------------------------------------------------------------------------
    try:
        alive = {(m["store_book_a"], m["store_book_b"]) for m in match_rows}
        old = db.fetch_auto_match_pairs(client)
        stale = [r["id"] for r in old
                 if (r["store_book_a"], r["store_book_b"]) not in alive]
        if stale:
            db.delete_matches(client, stale)
            print(f"  🧹 이제는 안 묶이는 옛 기록 {len(stale):,}건을 지웠습니다 "
                  f"(남은 기록 {len(old) - len(stale):,}건 + 새 기록). "
                  f"검토 화면이 이것 때문에 '자동으로 묶음' 이라고 "
                  f"거짓으로 적고 있었습니다.")
        else:
            print("  🧹 지울 옛 기록 없음 (검토 화면과 실제가 같습니다)")
    except Exception as exc:  # noqa: BLE001
        # 지우기가 실패해도 매칭 자체는 성공입니다. 조용히 넘기지 말고 알립니다.
        print(f"  ⚠️ 옛 기록 정리에 실패했습니다(매칭 결과에는 영향 없음): {exc}")
        print("     검토 화면에 이미 갈라진 짝이 남아 있을 수 있습니다.")
