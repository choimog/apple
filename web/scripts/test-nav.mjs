...
console.log("\n[1] 🚨 메뉴가 제 줄을 갖는가 (휴대폰 · PC 둘 다)");
// 【2026-08-19 대표님 지적】
//   "PC화면에서 상단 메뉴바에 메뉴가 늘어나면서 가로스크롤바가 생겼어."
//
// 재 본 값 (즐겨찾기가 늘어난 뒤):
//   메뉴에 필요한 폭            876px
//   제목·다크모드·로그아웃과 한 줄을 나눠 쓸 때 남는 자리  825px
//   → 51px 이 늘 모자랐습니다. 본문 폭이 1152px 로 묶여 있어서
//     화면을 아무리 키워도 안 늘어납니다 = **모든 PC 화면에 항상** 스크롤바.
//   메뉴에 줄 하나를 통째로 주면 1,120px → 244px 이 남습니다.
check("메뉴가 제목과 한 줄을 나눠 쓰지 않는다",
  !/hidden[^"]*sm:flex/.test(nav),
  "제목 옆에 두면 PC 에서 51px 이 모자라 스크롤바가 생깁니다");
check("메뉴 줄을 넓은 화면에서 감추지 않는다",
  !/sm:hidden/.test(nav),
  "감추면 PC 에 메뉴가 아예 없어집니다");
...
check("메뉴 줄은 얇게 (py-1)", /border-t border-line-soft px-1 py-1"/.test(nav),
  "붙박이 머리라 두꺼워지면 그만큼 본문이 가려집니다");
