...
class Candidate:
    ...
    # 【2026-08-18 대표님 지시 — 시리즈 권 번호】
    # 서점이 적어 놓은 그대로의 제목입니다. 권 번호는 여기서만 꺼낼 수
    # 있습니다. norm_title 은 띄어쓰기를 지워 놓아서 '과학책 2 육상생물' 의
    # 2 가 글자에 파묻힙니다.
    raw_title: str | None = None
    # 권 번호. **직접 넣지 마세요** — 아래에서 raw_title 로 계산합니다.
    volume: int | None = None

    def __post_init__(self) -> None:
        # 🚨 한 곳에서만 계산합니다. 만드는 자리마다 따로 넣게 하면
        #    한 군데를 빠뜨려도 화면에는 아무 표시가 안 납니다.
        if self.volume is None:
            self.volume = extract_volume(self.raw_title)
...
# ---------------------------------------------------------------------------
#  🚨 괄호 안이 '다른 이름' 이 아니라 '분류' 인 경우 — 2026-08-18
# ---------------------------------------------------------------------------
#  아래 publisher_variants 는 괄호 안을 **다른 이름**이라고 가정했습니다.
#
...
#      교보문고(교재)  ·  군자출판사(교재)
#
#  '참고서' 가 겹친다는 이유로 **지학사와 NE능률이 닮은 정도 1.00**,
#  즉 완전히 같은 출판사가 되고 있었습니다. 2026-08-18 매칭 기록에서
#  발견했습니다. 표기 6,696가지가 5,594곳으로 모였는데, 그중 한 덩어리가
#  이랬습니다.
#
#      학산문화사 ← NE능률 · 교보문고 · 시공사 · 지학사 · 메가스터디북스
#                   · 군자출판사 · 한빛아카데미 · 해냄에듀 · 키움 …
#
#  전혀 남남인 출판사 수십 곳이 한 줄로 합쳐졌습니다. 웰컴 화면의
#  [출판사 TOP 8] 과 출판사별 화면이 그대로 틀립니다. 매칭에서도
#  '출판사가 다르니 다른 책' 이라는 갈라내기가 이 짝들에서는 안 먹습니다.
#
#  🚨 확인 도구(verify_publishers)가 "0건" 이라고 한 이유도 이것입니다.
#     **같은 잣대**를 쓰기 때문에 자기 눈으로는 못 봅니다.
#
#  【고치는 법】 괄호 안이 아래 분류어면 '다른 이름' 후보로 쓰지 않습니다.
#     · 괄호 밖 이름('지학사')은 그대로 남습니다 → 예스24의 '지학사' 와는
#       여전히 1.00 으로 이어집니다. 잃는 것이 없습니다.
#     · 목록은 config/matching.yaml 에서 늘릴 수 있습니다. 다만 **여기
#       적어 둔 기본값이 항상 켜져 있습니다.** 설정을 안 읽는 곳에서
#       조용히 예전 상태로 돌아가면 안 되기 때문입니다.
# ---------------------------------------------------------------------------
_CATEGORY_DEFAULT = {
    # 책의 종류
    "단행본", "단행", "일반서", "전집", "만화", "라이트노벨", "잡지", "월간지",
    "무크", "사전", "화보집", "아트북",
    # 학습·수험
    "참고서", "교재", "교과서", "학습", "학습서", "문제집", "수험서", "자격증",
    "대학교재", "초등", "중등", "고등", "중고등", "유아", "아동", "어린이",
    "청소년",
    # 갈래
    "국내서", "외서", "외국어", "실용", "종교", "기독교", "컴퓨터", "예술",
    # 분류를 모르겠을 때 서점이 넣는 말
    "기타", "기타등등",
}

_CATEGORY: set[str] = set(_CATEGORY_DEFAULT)


def set_publisher_categories(words: list[str] | None) -> int:
    """
    괄호 안에 와도 '다른 이름' 으로 보지 않을 분류어를 등록합니다.

    ⚠️ 기본값을 **없애지 않고 더합니다.** 설정 파일에서 한 줄을 지웠다고
       지학사와 NE능률이 다시 같은 출판사가 되면 안 됩니다.
    """
    _CATEGORY.clear()
    _CATEGORY.update(_CATEGORY_DEFAULT)
    for w in words or ():
        w = (w or "").strip()
        if w:
            _CATEGORY.add(w)
    return len(_CATEGORY)


def publisher_categories() -> set[str]:
    return set(_CATEGORY)
...
def publisher_variants(name: str | None) -> set[str]:
    ...
    for chunk in inner:
        chunk = chunk.strip()
        # 🚨 괄호 안이 분류어면 '다른 이름' 후보로 쓰지 않습니다.
        #    (지학사(참고서) 와 NE능률(참고서) 가 '참고서' 로 이어지던 것)
        #    괄호 밖 이름은 위에서 이미 넣었으므로 잃는 것은 없습니다.
        if chunk and chunk not in _CATEGORY:
            out.add(chunk)
...
# -----------------------------------------------------------------------------
#  "청림Life 랑 청림라이프처럼, 서점마다 출판사를 표기하는 명칭이 조금씩
#   다른데 이것도 다 규칙화하기 어려울 것 같아서."
#
#  맞습니다. 한글/영문 표기 차이는 글자로는 절대 못 잡습니다
#  (청림Life ↔ 청림라이프 는 닮은 정도 0.24).
#
#  🚨 【왜 여기 한 군데에만 두나요?】
...
def compare(a: Candidate, b: Candidate, cfg: dict) -> MatchResult:
    ...
    #
    #  🚨 【2026-08-18 — 재는 자리를 옮겼습니다. 갈라내는 규칙은 그대로입니다】
    #  예전에는 여기서 바로 갈라냈습니다. 제목을 보기도 **전**이었습니다.
    #  그러면 남남인 두 책(제목 앞 네 글자만 같은 책)도 "정가가 다름" 으로
    #  갈라집니다. 실제로 그렇게 세었더니 이런 표가 나왔습니다.
    #
    #      🚨 교보↔예스24: 89,115 / 141,900쌍 (62.8%)
    #      ✅ 교보↔교보:        0 /  98,466쌍 (0.0%)   ← 이 줄이 증거입니다
    #
    #  같은 서점끼리는 맨 첫 줄에서 갈라지므로 정가까지 오지도 못합니다.
    #  그런데 분모에는 들어 있었습니다. **분모가 '같은 책 후보' 가 아니라
    #  '비교해 본 모든 짝' 이었던 것입니다.** 62.8% 는 "정가가 이상하다" 가
    #  아니라 "비교해 본 짝의 대부분이 원래 다른 책이다" 라는 뜻이었습니다.
    #  (같은 날 db/price-check.sql 로 실제 자료를 세었을 때는 0.77% 였습니다)
    #
    #  그래서 **갈라내는 일은 다른 조건이 전부 같은 것을 확인한 뒤**에
    #  합니다. 갈라지는 짝은 예전과 똑같습니다. 달라지는 것은 두 가지입니다.
    #    · 기록되는 이유가 더 정확해집니다 ("제목이 너무 다름" 등)
    #    · 로그의 정가 점검 비율이 **진짜 뜻하는 값**이 됩니다
    # -------------------------------------------------------------------------
    ...
    # =========================================================================
    #  1단계의 마지막 — '나머지는 다 같은데 이것만 다른' 두 가지
    # =========================================================================
    #  여기까지 왔다는 것은 제목·저자쪽 문턱·출판사·출간월·에디션·세트가
    #  **전부 통과**했다는 뜻입니다. 그러니 아래 둘은 정말로 '이것만 다른'
    #  경우입니다. 로그에 찍히는 건수도 그제야 뜻이 생깁니다.
    #  (2026-08-18 에 옮겼습니다. 위 정가 설명의 62.8% 사고 참고)

    # -------------------------------------------------------------------------
    #  ① 시리즈 권 번호가 다르면 다른 책입니다 — 2026-08-18 대표님 지시
    #
    #  "두 도서가 시리즈로 유사한 제목과 넘버링을 가지고 있는 경우에는,
    #   넘버링이 다르면 명확하게 다른 도서로 구분지어줄 필요가 있을 거거든?"
    #
    #  검토 화면에 올라와 있던 실제 짝입니다. 둘 다 **89점**이었습니다.
    #     수상한생선의 진짜로 해부하는 과학책 1        (알라딘)
    #     수상한 생선의 진짜로 해부하는 과학책 2 육상생물 (예스24)
    #     빛과 수의 시대 1 (알라딘)  /  빛과 수의 시대 2 (교보)
    #
    #  저자·출판사·출간월이 전부 같고 제목도 78~79% 닮아서, 점수로는
    #  **묶는 것이 맞다고 나옵니다.** 번호만이 유일한 차이인데 아무도
    #  그 번호를 안 보고 있었습니다. 그래서 점수 위에 놓습니다.
    #
    #  ⚠️ **양쪽 다 번호가 있을 때만** 갈라냅니다.
    #     1권은 번호 없이 파는 일이 흔합니다. 한쪽에 번호가 없다고
    #     '1권' 으로 치면 『해리포터와 마법사의 돌』 과
    #     『해리포터와 마법사의 돌 1』 이 갈라집니다. 모르는 것은 모르는
    #     대로 둡니다 (출간월·정가와 같은 원칙입니다).
    # -------------------------------------------------------------------------
    if a.volume is not None and b.volume is not None and a.volume != b.volume:
        if th.get("volume_hard", True):
            return _reject("권 번호가 다름", {"a": a.volume, "b": b.volume})

    # ---- ② 정가 (규칙 설명은 위 same_price 자리에 있습니다) ----
    if same_price is False:
        ...
        if th.get("price_hard", True) and both_trusted:
    ...
    # 🚨 권 번호도 '같았다' 를 남깁니다. 이게 없으면 로그의 분모가
    #    '갈라낸 짝' 뿐이라 비율이 언제나 100% 가 됩니다 (= 아무 뜻 없음).
    if a.volume is not None and b.volume is not None:
        reasons["volume"] = f"same({a.volume})"
    elif a.volume is not None or b.volume is not None:
        reasons["volume"] = "한쪽만 있음"
    if prefix:
