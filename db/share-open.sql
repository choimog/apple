...
-- ---------------------------------------------------------------------------
--  5. 🚨 바뀐 것을 사이트에 알립니다 — 이걸 빼먹으면 화면이 안 바뀝니다
-- ---------------------------------------------------------------------------
--  【2026-08-10 대표님 신고 — "했는데 왜 '아직 준비가 안 됐습니다' 가 뜨지?"】
--
--  Supabase 는 표와 함수의 목록을 **따로 외워 두고** 씁니다. 위에서
--  my_share_links 를 지웠다가 새로 만들면(칸이 늘었으니 그래야 합니다),
--  외워 둔 목록에는 아직 **없어진 옛 함수**가 남아 있습니다.
--  그래서 사이트가 부르면 "그런 함수 없다" 는 답이 돌아옵니다.
--
--  아래 한 줄이 "목록을 다시 외워라" 라는 뜻입니다. 몇 초면 됩니다.
--  (Supabase 화면에서 Settings → API → Reload schema 를 눌러도 같습니다)
-- ---------------------------------------------------------------------------
NOTIFY pgrst, 'reload schema';


-- ---------------------------------------------------------------------------
--  6. 확인 — 아래 5줄이 전부 ✅ 면 끝입니다
-- ---------------------------------------------------------------------------
SELECT * FROM (
    ...
    UNION ALL
    -- 🚨 목록 함수가 **새 모양**인지 봅니다.
    --    '있다' 만 확인하면, 옛 모양이 남아 있는 것을 못 잡습니다.
    --    (여기서 함수를 직접 부르면 안 됩니다 — SQL Editor 에는 로그인
    --     정보가 없어서 '로그인이 필요합니다' 로 멈춰 버립니다)
    SELECT 5, '링크 목록이 새 모양으로 바뀜',
           CASE WHEN to_regprocedure('my_share_links()') IS NOT NULL
                 AND pg_get_function_result(to_regprocedure('my_share_links()'))
                     LIKE '%owner_email%'
